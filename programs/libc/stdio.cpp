...
void printf(const char *fmt, ...)
{
    ...
    for (size_t i = 0; fmt[i] != '\0'; i++)
    {
        ...
        auto emit_padded = [&](const char *s, int slen)
        {
            ...
            if (!flag_left)
            {
                for (int p = 0; p < pad; p++) PutChar(pc);
            }

            for (int p = 0; p < slen; p++) PutChar(s[p]);
        ...
        switch (spec)
        {
            ...
            case 'i':
            {
                ...
                if (!flag_left) for (int p = 0; p < pad; p++) PutChar(pc);
                if (sign) PutChar(sign);
                for (int p = 0; buf[p]; p++) PutChar(buf[p]);
                if (flag_left) for (int p = 0; p < pad; p++) PutChar(' ');
            } break;
            ...
            case 'X':
            {
                ...
                if (!flag_left) for (int p = 0; p < pad; p++) PutChar(pc);
                if (flag_hash && val != 0) { PutChar('0'); PutChar(spec == 'X' ? 'X' : 'x'); }
                for (int p = 0; buf[p]; p++) PutChar(buf[p]);
                if (flag_left)  for (int p = 0; p < pad; p++) PutChar(' ');
            } break;
            ...
            case 'c':
            {
                ...
                {
                    for (int p = 0; p < width - 1; p++) PutChar(' ');
                }
                PutChar(c);
                if (width > 1 && flag_left)
                {
